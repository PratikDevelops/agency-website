    ...
    <>
      <header
        ...
        style={{
          ...
          borderBottom: scrolled ? '1px solid var(--color-mist)' : 'none',
        }}
      >
        <div className="container-page">
          <div className="flex items-center justify-between">
            ...
            <Link
              href="/"
              className="flex items-center gap-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ember-orange"
            >
              <AppLogo size={32} />
              <span
              ...
              <div className="nav-pill flex items-center gap-1">
                {navLinks.map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className={`font-polysans px-4 py-2 transition-all duration-200 rounded-full text-sm ${
                      isActive(link.href)
            ...
            <nav className="flex flex-col gap-1">
              {navLinks.map((link, i) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`font-polysans text-2xl py-4 border-b border-mist transition-colors ${
                    isActive(link.href) ? 'text-ember-orange' : 'text-graphite hover:text-steel'
                  }`}
                  style={{
