      ...
      <div className="container-page">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {plans?.map((plan, i) => (
            <div
              key={plan?.name}
              className={`reveal p-8 rounded-lg flex flex-col gap-6 relative ${
                plan?.popular ? 'bg-graphite text-white' : 'bg-ash'
              }`}
            ...
            >
              ...
              <div>
                ...
                <p
                  className={`font-inter text-sm ${plan?.popular ? 'text-white opacity-70' : 'text-slate'}`}
                >
                  {plan?.tagline}
                </p>
              </div>
              ...
              <ul className="flex flex-col gap-3 flex-1">
                {plan?.features?.map((f) => (
                  <li key={f} className="flex items-start gap-2">
                    <span
                      className={`mt-1 flex-shrink-0 ${plan?.popular ? 'text-ember-orange' : 'text-ember-orange'}`}
                    >
                    ...
                    <span
                      className={`font-inter text-sm ${plan?.popular ? 'text-white opacity-80' : 'text-steel'}`}
                    >
                      {f}
                    </span>
                  </li>
              ...
              <Link
                ...
                style={
                  plan?.popular ? { borderColor: 'rgba(255,255,255,0.3)', color: 'white' } : {}
                }
              >
                Get Started
              </Link>
